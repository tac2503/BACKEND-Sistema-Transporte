...
from src.core.config import ACCESS_TOKEN_EXPIRE_MINUTES, JWT_ALGORITHM, get_jwt_secret_key
...
def authenticate_admin(db: Session, documento: str, contrasena: str) -> Administrador | None:
...
def authenticate_cliente(db: Session, documento: str, contrasena: str) -> Cliente | None:
...
def get_current_admin(
	...
	admin = db.query(Administrador).filter(Administrador.documento == payload["sub"]).first()
	if not admin:
		raise HTTPException(
			status_code=status.HTTP_401_UNAUTHORIZED,
